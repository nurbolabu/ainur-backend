    ...
    <div className="animate-in fade-in duration-300">
      <div className="flex justify-between items-center pr-4 md:pr-0 mb-4">
        ...
        <label className="btn-text cursor-pointer">
           {isUploading ? <Loader2 size={24} className="animate-spin text-black"/> : 'Добавить'}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 ios-bubble-margin">
        {stories.map(s => (
          <div key={s.id} className="ios-bubble mb-0 aspect-[9/16] relative bg-black">
            ...
            <button onClick={() => handleDelete(s.id)} className="absolute top-3 right-3 w-10 h-10 bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center text-white"><Trash2 size={20} /></button>
          </div>
        ))}
      </div>
    </div>
