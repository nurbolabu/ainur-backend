  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex justify-between items-center pr-4 md:pr-0">
        ...
        <button onClick={handleSave} className="btn-text font-semibold mb-4">{isSaving ? '...' : 'Сохранить'}</button>
      </div>
      ...
      <div className="ios-bubble ios-bubble-margin">
        <div className="ios-list-row bg-white cursor-default">
          ...
          <input type="text" className="input-bare text-right" value={formData.company_name} onChange={e => setFormData({...formData, company_name: e.target.value})} />
        </div>
        <div className="ios-list-row bg-white cursor-default">
          <span className="w-[120px] text-[17px]">Лого (URL)</span>
          <input type="text" className="input-bare text-right" value={formData.logo_url} onChange={e => setFormData({...formData, logo_url: e.target.value})} />
        </div>
        <div className="ios-list-row bg-white cursor-default">
          ...
          <div className="flex items-center gap-3">
             ...
             <input type="color" value={formData.theme_color} onChange={e => setFormData({...formData, theme_color: e.target.value})} className="w-8 h-8 rounded-full border-none p-0 cursor-pointer bg-transparent" />
          </div>
        </div>
      </div>
      ...
      <div className="ios-bubble ios-bubble-margin mb-8">
        <div className="p-4 bg-white border-b border-[#C6C6C8]">
          ...
          <textarea rows={2} className="input-bare resize-none" value={formData.system_prompt} onChange={e => setFormData({...formData, system_prompt: e.target.value})} />
        </div>
        <div className="p-4 bg-white border-b border-[#C6C6C8]">
          <span className="block text-[15px] text-[#8E8E93] mb-1">Знания для ИИ</span>
          <textarea rows={4} className="input-bare resize-none" value={formData.knowledge_base} onChange={e => setFormData({...formData, knowledge_base: e.target.value})} />
        </div>
        <div className="p-4 bg-white">
          <span className="block text-[15px] text-[#8E8E93] mb-1">Приветствие</span>
          <input type="text" className="input-bare" value={formData.welcome_message} onChange={e => setFormData({...formData, welcome_message: e.target.value})} />
        </div>
      </div>
    </div>
  );
