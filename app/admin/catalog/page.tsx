...
import { Plus, Trash2, Edit, Image as ImageIcon, ShoppingBag, Upload, X, Loader2 } from 'lucide-react';
...
export default function CatalogPage() {
  ...
  // Состояние формы (добавили массив для нескольких фото и ID для редактирования)
  ...
  // --- ЗАГРУЗКА КАРТИНОК В SUPABASE STORAGE ---
  ...
  async function handleSaveProduct() {
    ...
    const productData = {
      ...
      image_url: newProduct.image_urls.join(','), // Склеиваем массив в строку
    ...
    <div className="animate-in fade-in duration-500 pb-20">
     <header className="mb-8 flex flex-col md:flex-row md:justify-between items-start md:items-end gap-4">
        ...
        <div className="bg-white/80 border border-white p-6 rounded-[24px] shadow-sm mb-8 flex flex-col gap-4">
          ...
          <div className="mt-2">
            ...
            <div className="flex flex-wrap gap-4 items-start">
              ...
              <label className="w-24 h-24 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((product) => {
          ...
          <div key={product.id} className="bg-white border border-gray-100 p-5 rounded-[24px] shadow-sm flex flex-col">
            ...
            <div className="flex gap-2 mt-6 pt-4 border-t border-gray-100">
              <button onClick={() => startEditing(product)} className="flex-1 py-2 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 font-medium"><Edit size={16}/> Редактировать</button>
        ...
        {products.length === 0 && !isAdding && (<div className="col-span-full py-12 flex flex-col items-center text-gray-400 border-2 border-dashed border-gray-200 rounded-[24px]"><ShoppingBag size={48} className="mb-4 opacity-50" /><p>Каталог пуст</p></div>)}
      </div>
    </div>
  );
}