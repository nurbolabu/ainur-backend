...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (mode !== 'update' && !email.trim()) { setError('Введите Email'); return; }
    if (mode === 'login' && !password.trim()) { setError('Введите пароль'); return; }
    ...
    try {
      if (mode === 'login') {
        ...
        if (data.user) {
          const { data: project } = await supabase.from('projects').select('id').eq('user_id', data.user.id).maybeSingle();
      ...
      <div className="w-full max-w-[420px] bg-white rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#F0F0F0] p-8 md:p-10 relative z-20">
        ...
        {mode === 'login' && (
          <div className="mt-5 text-center">
            <button type="button" onClick={() => { setMode('forgot'); setError(''); setSuccessMsg(''); }} className="text-[14px] font-semibold text-black hover:underline transition-all">Забыли пароль?</button>
          </div>
        )}

        {(mode === 'forgot' || mode === 'update') && (
          <div className="mt-6 text-center">
            <button onClick={() => { setMode('login'); setError(''); setSuccessMsg(''); }} className="flex items-center justify-center gap-1.5 text-[14px] font-semibold text-[#8E8E93] hover:text-black transition-colors mx-auto">
